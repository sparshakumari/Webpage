...
%##################################################################################################################
%########################### CUSTOM CODE ##############################
[fs, low_frequency, high_frequency, t_l, t_h] = input_parameters();         %Calling the input box
...
elseif low_frequency <= 0
    ...
    waitfor(f);
    [fs, low_frequency, high_frequency, t_l, t_h] = input_parameters();
...
elseif t_l >= t_h
    f = errordlg('Higher time limit must be greater than the lower time limit','Input Error');
    waitfor(f);
    [fs, low_frequency, high_frequency, t_l, t_h] = input_parameters();

elseif t_l < 0
    f = errordlg('Lower time limit must be greater than 0','Input Error');
    waitfor(f);
    [fs, low_frequency, high_frequency, t_l, t_h] = input_parameters();
end
...
plot(time (:,1),fdata(:,1),'LineWidth', 2.0, 'color', 'k')
xlim([t_l t_h])
ylim([-0.3 0.3])
...
sdata = fdata(find(abs(time - t_l)< 0.001):find(abs(time - t_h)< 0.001));   % tolerance value added 
[pxx,f] = pwelch(sdata, hamming(length(sdata)),0,length(sdata),fs);

figure('WindowState','maximized','Color',[1 1 1])
plot(f,pxx,'LineWidth',2.0, 'color', 'k')
xlim([0 20])
...
%########################### PLOT SPECTROGRAM ##############################
% [~, F1, T1, P1] = spectrogram(sdata, 250, 240, 2000, fs);
% figure('WindowState','maximized','Color',[1 1 1]);
% surf(T1, F1, P1, 'edgecolor' , 'none');
...
function [fs, low_frequency, high_frequency, t_l, t_h] = input_parameters()
answer = inputdlg({'Sampling frequency','Lower cut-off frequency','Higher cut-off frequency', 'Lower time limit','Upper time limit'}, 'Input parameters', [1 75; 1 75; 1 75; 1 75; 1 75]);
fs = str2double(answer{1, :});
low_frequency = str2double(answer{2, :});
high_frequency = str2double(answer{3, :});
t_l = str2double(answer{4, :});
t_h = str2double(answer{5, :});
return;
