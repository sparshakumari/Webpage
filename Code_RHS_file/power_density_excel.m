...
%##################################################################################################################
%########################### CUSTOM CODE ##############################
[fs, low_frequency, high_frequency] = input_parameters();         %Calling the input box

if fs <= 0
    f = errordlg('Sampling frequency should be a positive number','Input Error');
    waitfor(f);
    [fs, low_frequency, high_frequency] = input_parameters();         %Calling the input box

elseif low_frequency <= 0
    f = errordlg('Lower cut-off frequency should be a positive number','Input Error');
    waitfor(f);
    [fs, low_frequency, high_frequency] = input_parameters();

elseif low_frequency >= high_frequency
    f = errordlg('Higher cut-off frequency must be greater than the lower cut-off frequency','Input Error');
    waitfor(f);
    [fs, low_frequency, high_frequency] = input_parameters();
end

%########################### RAW DATA ##############################
amplitudes = amplifier_data(1,:);
amplitudes = transpose(amplitudes);
amplitudes = 1/1000 * amplitudes;
t = transpose(t);

dummy = t(1,1) .* ones(length(t),1);
t = t - dummy;

[time, data] = decimateD(amplitudes, t, 20);
fprintf(1, 'Raw data is downsampled.\n');

%########################### FILTER DATA ##############################
fdata = bandpass_filter(data,low_frequency, high_frequency, fs);
fprintf(1, 'Bandpass filter from %0.1f Hz to %0.1f Hz was applied to the data.\n',low_frequency, high_frequency);
fprintf(1, '\n');

%######################## WRITING POWER VALUES TO EXCEL ################
answer = inputdlg({'Number of time ranges to be input:'}, 'Time range', [1 75]);
n = str2double(answer{1, :});

recycle on % Send to recycle bin instead of permanently deleting.
delete('powerdensity.xlsx'); 

for x = 1:n
    a = inputdlg({'Lower limit (s)', 'Higher limit (s)'}, sprintf('Time range - %d',x), [1 75; 1 75]);
    timel = str2double(a{1, :});
    timeh = str2double(a{2, :});
    if timel >= timeh
        f = errordlg('Lower time limit must be less than the upper time limit','Input Error');
        waitfor(f);
        a = inputdlg({'Lower limit (s)', 'Higher limit (s)'}, sprintf('Time range - %d',x), [1 75; 1 75]);
        timel = str2double(a{1, :});
        timeh = str2double(a{2, :});
    end
    sdata = fdata(find(abs(time - timel)< 0.001):find(abs(time - timeh)< 0.001));   % tolerance value added 
    [pxx,f] = periodogram(sdata,hamming(length(sdata)),length(sdata),fs);
    xlswrite ('powerdensity.xlsx',f,[xlsAddr(1,1) ':' xlsAddr(size(f,1),1)])
    xlswrite ('powerdensity.xlsx',pxx,[xlsAddr(1,(x+1)) ':' xlsAddr(size(f,1),(x+1))])
    fprintf(1, 'Power values for the time range %0.1f s to %0.1f s is written to the excel spreadsheet.\n', timel,timeh);
end
fprintf(1, '\n');
fprintf(1, 'Successful!\n');
fprintf(1, 'The power values are now available in the excel spreadsheet.\n');
%######################### CUSTOM CODE ENDS ##################################
return
...
function [fs, low_frequency, high_frequency] = input_parameters()
answer = inputdlg({'Sampling frequency','Lower cut-off frequency','Higher cut-off frequency'}, 'Input parameters', [1 75; 1 75; 1 75]);
fs = str2num(answer{1, :});
low_frequency = str2num(answer{2, :});
high_frequency = str2num(answer{3, :});
return;
